      ...
      <div className="max-w-7xl mx-auto p-4 md:p-6 space-y-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="bg-white p-8 rounded-3xl shadow border text-center">
            <img className="w-40 h-40 rounded-full mx-auto object-cover border-4 border-sky-500"
              src={ agent.img ? `https://lightblue-moose-690494.hostingersite.com/public${agent.img}` : "https://via.placeholder.com/150" }
            />
        ...
        <div>
          ...
          {paginated.length === 0 ? (
            ...
            <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              {paginated.map((item) => (
                <div key={item.id} className="bg-white rounded-2xl border shadow hover:shadow-xl transition overflow-hidden">
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex justify-between items-center mt-3">
                      ...
                      <button onClick={() => navigate(`/property/${item.id}`)} className="px-3 py-1 bg-sky-500 text-white rounded-lg text-xs">
                        View
                      </button>
                    </div>
                  </div>
          ...
          {totalPages > 1 && (
            <div className="flex justify-center gap-3 mt-8">
              <button onClick={() => setPage((p) => Math.max(p - 1, 1))} disabled={page === 1} className="px-4 py-2 border rounded-lg">
                Prev
              </button>
              ...
              <button onClick={() => setPage((p) => Math.min(p + 1, totalPages))} disabled={page === totalPages} className="px-4 py-2 border rounded-lg">
                Next
              </button>
            </div>
          )}
